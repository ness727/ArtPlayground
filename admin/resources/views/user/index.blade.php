...
<div class="w-full overflow-x-hidden border-t flex flex-col">
	...
	<div class="flex justify-between items-center">
        <h1 class="text-3xl font-bold text-black">사용자 관리</h1>
        ...
        <div class="w-full mt-4">
			<div class="flex justify-end">
				<a class=" my-4 bg-transparent leading-tight hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent" href="{{ route('user.create') }}">추가</a>
			</div>
                ...
                <table class="min-w-full bg-white">
                    <thead class="bg-gray-800 text-white">
                        <tr>
                            ...
                            <th class="text-left py-3 px-4 uppercase font-semibold text-sm">User_id</th>
                    ...
                    <tbody class="text-gray-700">
						@foreach ($users as $user)
							<tr>
								...
								<td class="text-left py-3 px-4">{{ $user->user_id }}</td>
								<td class="text-left py-3 px-4">암호화됨</td>
								<td class="text-left py-3 px-4">{{ $user->address }}</td>
								<td class="text-left py-3 px-4">{{ $user->status }}</td>
								<td class="py-3 flex justify-center">
									<a class="font-bold hover:text-blue-800 w-16 px-4 mr-4 text-blue-500" href="{{ route('user.edit', $user->id) }}{{ $tmp }}">수정</a>
		...
		<div class="m-4 flex justify-center">
			{{ $users->appends(['text1' => $text1])->links('mypagination') }}
		</div>
    </main>
</div>
