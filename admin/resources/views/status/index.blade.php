...
<div class="w-full overflow-x-hidden border-t flex flex-col">
	...
	<div class="flex justify-between items-center">
        <h1 class="text-3xl font-bold text-black">상태 관리</h1>
        ...
        <div class="mt-4">
			<div class="flex justify-end">
				<a class="my-4 bg-transparent leading-tight hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent" href="{{ route('status.create') }}">추가</a>
			</div>
                ...
                <table class="min-w-full bg-white">
                    ...
                    <tbody class="text-gray-700">
						@foreach ($statuss as $status)
							<tr>
								<td class="text-left py-3 px-4">{{ $status->id }}</td>
								<td class="text-left py-3 px-4">{{ $status->name }}</td>
								<td class="py-3 flex justify-center">
									<form id="deleteForm" action="{{ route('status.destroy', $status->id) }}" method="post">
										@csrf
										@method('DELETE')
		...
		<div class="m-4 flex justify-center">
			{{ $statuss->appends(['text1' => $text1])->links('mypagination') }}
		</div>
    </main>
</div>
