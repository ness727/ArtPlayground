...
<script>
	function find_text() {
		form1.action = "{{ route('status.index') }}";
		form1.submit();
	}
</script>
...
<div class="w-full overflow-x-hidden border-t flex flex-col">
	...
	<div class="flex justify-between items-center">
        <h1 class="text-3xl font-bold text-black">굿즈 관리</h1>
        ...
        <div class="w-full mt-4">
			<div class="flex justify-end">
				<a class="my-4 bg-transparent leading-tight hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent" href="{{ route('goods.create') }}">추가</a>
			</div>
                ...
                <table class="min-w-full bg-white">
                    ...
                    <tbody class="text-gray-700">
						@foreach ($goods as $goods)
							<tr>
								<td class="text-left py-3 px-4">{{ $goods->id }}</td>
								<td class="text-left py-3 px-4">{{ $goods->name }}</td>
								<td class="text-left py-3 px-4">{{ $goods->img }}</td>
								<td class="text-left py-3 px-4">{{ $goods->price }}</td>
								<td class="text-left py-3 px-4 w-1/2 line-clamp-2">{{ $goods->description }}</td>
								<td class="py-3 flex justify-center h-full">
									<a class="font-bold hover:text-blue-800 px-4 mr-4 text-blue-500" href="{{ route('goods.edit', $goods->id) }}{{ $tmp }}">수정</a>
									<form id="deleteForm" action="{{ route('goods.destroy', $goods->id) }}" method="post">
										@csrf
										@method('DELETE')
		...
		<div class="m-4 flex justify-center">
			{{ $goods->appends(['text1' => $text1])->links('mypagination') }}
		</div>
    </main>
</div>
