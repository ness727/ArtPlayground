...
<div class="w-full overflow-x-hidden border-t flex flex-col">
	...
	<div class="flex justify-between items-center">
        <h1 class="text-3xl font-bold text-black">전시회 관리</h1>
        ...
        <div class="w-full mt-4">
			<div class="flex justify-end">
				<a class=" my-4 bg-transparent leading-tight hover:bg-blue-500 text-blue-700 font-semibold hover:text-white py-2 px-4 border border-blue-500 hover:border-transparent" href="{{ route('exhibition.create') }}">추가</a>
			</div>
                ...
                <table class="min-w-full bg-white">
                    <thead class="bg-gray-800 text-white">
                        <tr>
							...
							<th class="text-left py-3 px-4 uppercase font-semibold text-sm w-1/2">Description</th>
                    ...
                    <tbody class="text-gray-700">
						@foreach ($exhibitions as $exhibition)
							<tr>
								<td class="text-left py-3 px-4">{{ $exhibition->id }}</td>
								<td class="text-left py-3 px-4">{{ $exhibition->title }}</td>
								<td class="text-left py-3 px-4">{{ $exhibition->img }}</td>
								<td class="text-left py-3 px-4">{{ $exhibition->start_date }}</td>
								<td class="text-left py-3 px-4">{{ $exhibition->end_date }}</td>
								<td class="text-left py-3 px-4 w-1/2 line-clamp-2">{{ $exhibition->description }}</td>
								<td class="text-left py-3 px-4">{{ $exhibition->price }}</td>
								<td class="py-3 flex justify-center h-full">
									<a class="font-bold hover:text-blue-800 w-16 px-4 mr-4 text-blue-500" href="{{ route('exhibition.edit', $exhibition->id) }}{{ $tmp }}">수정</a>
									<form id="deleteForm" action="{{ route('exhibition.destroy', $exhibition->id) }}" method="post">
										@csrf
										@method('DELETE')
