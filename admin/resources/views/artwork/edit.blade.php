...
<form name="form1" method="post" action="{{ route('artwork.update', $artwork->id) }}{{ $tmp }}" class="flex flex-col items-center justify-center py-4" enctype="multipart/form-data">
@csrf
@method('PUT')
	<div class="w-2/3">
		<div class="font-bold text-2xl py-4">그림 수정</div>
	</div>
    <table class="bg-white w-2/3">
		...
		<tr>
			...
			<td class="text-left py-3 px-4">
				<input name="title" value="{{ $artwork->title }}" class="h-8 w-full bg-gray-100" />
		...
		<tr>
			...
			<td class="text-left py-3 px-4">
				이전 파일: {{ $artwork->img }}
				<img src="{{ asset('/storage/artwork_imgs/' . $artwork->img) }}" class="w-1/6 h-1/6 py-2" />
		...
		<tr>
			...
			<td class="text-left py-3 px-4">
				<input name="year" value="{{ $artwork->year }}" class="h-8 w-full bg-gray-100" />
			</td>
		</tr>
		<tr>
			<th class="bg-gray-800 text-white text-left py-3 px-4 uppercase font-semibold text-sm">description</th>
			<td class="text-left py-3 px-4">
				<textarea name="description"class="p-4 h-40 w-full bg-gray-100">{{ $artwork->description }}</textarea>
			</td>
		</tr>
		<tr>
