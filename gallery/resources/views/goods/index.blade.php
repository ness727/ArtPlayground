          ...
          <div class="w-full px-4">
            <div class="mx-auto max-w-[700px] text-center">
              <h2 class="mb-12 text-3xl font-bold md:leading-[1.44] text-white md:text-[38px] ">
                <span class="dark:text-dark"> 미술가게 </span>
              </h2>
			  <h2 class=" text-2xl font-bold md:leading-[1.44] text-white md:text-[24px] ">
                <span class="dark:text-dark">다양한 그림과 작가에 대한 상품을 둘러보세요! </span>
              </h2>
            </div>
          </div>
        </div>
    ...
    <div class="container  my-28 mx-auto  max-w-screen-lg py-5 lg:py-8">
        ...
        <div class="mt-10 grid gap-10 md:grid-cols-2 lg:gap-10 xl:grid-cols-3 ">
			@foreach($goods as $goods)
				<div class="group cursor-pointer" onclick="showModalAndSetImage('{{ asset('../../admin/public/storage/goods_imgs/' . $goods->img) }}', '{{ $goods->title }}', '{{ $goods->description }}');">
					<div class=" overflow-hidden rounded-md bg-gray-100 transition-all hover:scale-105   dark:bg-gray-800">
						<div class="relative block aspect-square">
							<img alt="Thumbnail" loading="lazy" decoding="async" data-nimg="fill" class="object-cover transition-all" sizes="(max-width: 768px) 30vw, 33vw" srcset="{{ asset('../../admin/public/storage/goods_imgs/' . $goods->img) }}" style="position: absolute; height: 100%; width: 100%; inset: 0px; color: transparent;">
						</div>
					</div>
					<div>
						<div>
							...
							<h2 class="flex justify-between text-lg font-semibold leading-snug tracking-tight mt-2 dark:text-white">
								...
								<div onclick="
									showGptModalAndSetImage('{{ asset('../../admin/public/storage/goods_imgs/' . $goods->img) }}', '{{ $goods->name }}', '{{ $goods->price }}');
