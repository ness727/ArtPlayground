          ...
          <div class="w-full px-4">
            <div class="mx-auto max-w-[570px] text-center">
              <h2 class="mb-2.5 text-3xl font-bold md:leading-[1.44] text-white md:text-[38px] ">
                <span class="dark:text-dark">회원가입 페이지</span>
          ...
          <div
            ...
            <form action="{{ route('register') }}" method="POST">
			  ...
			  @if ($errors->has('password'))
				<div>
					<p style="color:#cd5c5c;">{{ $errors->first('password') }}</p>
					@if (count($errors->get('password')) > 1)
						<p style="color:#cd5c5c;">{{ $errors->get('password')[1] }}</p>
					@endif
				</div>
			@endif
              <div class="mb-[22px]">
			 ...
			 <div class="mb-[22px]">
				<p class="text-dark-6"> 영문과 숫자가 포함된 6자리 이상으로 설정해주세요.</p>
			 </div>							
										
			   <div class="mb-[22px]">
				<input type="text" name="zipcode" id="sample4_postcode" placeholder="우편번호"  class="mt-4 border-stroke dark:border-dark-3 bg-transparent w-2/3 rounded-md border py-2 px-2 text-base text-body-color dark:text-dark-6 placeholder:text-dark-6 outline-none transition focus:border-primary dark:focus:border-primary focus-visible:shadow-none">
				 <input type="button" onclick="sample4_execDaumPostcode()" value="우편번호 찾기" class="border-primary w-1/3 cursor-pointer rounded-md border bg-primary py-2 px-1  text-base text-white transition duration-300 ease-in-out hover:bg-blue-dark"><br>
				<input type="text" name="address1" id="sample4_roadAddress" placeholder="도로명주소" class="mt-1 border-stroke dark:border-dark-3 bg-transparent w-2/3 rounded-md border py-2 px-2 text-base text-body-color dark:text-dark-6 placeholder:text-dark-6 outline-none transition focus:border-primary dark:focus:border-primary focus-visible:shadow-none">
				<input type="hidden" id="sample4_jibunAddress" placeholder="지번주소">
                <span id="guide" style="color:#999;display:none"></span>
				<input type="text" name="address2" id="sample4_detailAddress" placeholder="상세주소" class="mt-1 border-stroke dark:border-dark-3 bg-transparent w-2/3 rounded-md border py-2 px-2 text-base text-body-color dark:text-dark-6 placeholder:text-dark-6 outline-none transition focus:border-primary dark:focus:border-primary focus-visible:shadow-none">
				<input type="text" name="address3" id="sample4_extraAddress" placeholder="참고항목" class="mt-1 border-stroke dark:border-dark-3 bg-transparent w-2/3 rounded-md border py-2 px-2 text-base text-body-color dark:text-dark-6 placeholder:text-dark-6 outline-none transition focus:border-primary dark:focus:border-primary focus-visible:shadow-none">
              </div>
