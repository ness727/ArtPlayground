	...
	<div id="profile" class="w-full lg:w-3/5 rounded-lg lg:rounded-l-lg lg:rounded-r-none shadow-2xl bg-white opacity-75 mx-6 lg:mx-0">
		...
		<div class="p-4 md:p-12 text-center lg:text-left">
			...
			@method('PATCH')
				...
				<h1 class="text-3xl font-bold pt-8 lg:pt-0">My Page</h1>
				<div class="mx-auto lg:mx-0 w-4/5 pt-3 border-b-2 border-green-500 opacity-25"></div>
				<div class="pt-4 flex">
					<p class="mr-2 text-base font-bold flex items-center justify-center lg:justify-start">이름:</p>
					<input class="pl-2 bg-slate-300 rounded-md" name="name" value="{{ $user->name }}"/>
				</div>
				<div class="pt-4 flex">
					<p class="mr-2 text-base font-bold flex items-center justify-center lg:justify-start">아이디:</p>
					<input class="pl-2 bg-slate-100 rounded-md" name="user_id" value="{{ $user->user_id }}" readonly />
				</div>
				<div class="pt-4 flex">
					<p class="mr-2 text-base font-bold flex items-center justify-center lg:justify-start">비밀번호:</p>
					<input class="pl-2 bg-slate-300 rounded-md" name="password" value="{{ $user->password }}"/>
				</div>
				<div class="pt-4 flex">
					<p class="mr-2 text-base font-bold flex items-center justify-center lg:justify-start">이메일:</p>
					<input class="pl-2 bg-slate-300 rounded-md" name="email" value="{{ $user->email }}"/>
				</div>
				<div class="pt-4 flex">
					<p class="mr-2 text-base font-bold flex items-center justify-center lg:justify-start">핸드폰 번호:</p>
					<input class="pl-2 bg-slate-300 rounded-md" name="phone" value="{{ $user->phone }}"/>
				</div>
				<div class="pt-4 flex flex-col gap-2">
					<p class="mr-2 text-base font-bold flex items-center lg:justify-start">주소:</p>
					<div class="flex justify-between">
						<!-- <input name="zipcode" id="sample4_postcode" class="pl-2 bg-slate-100 rounded-md w-20" value="{{ $user->zipcode }}" readonly /> -->
						<input name="address" id="sample4_roadAddress" class="pl-2 bg-slate-100 rounded-md mx-2 flex-grow" value="{{ $user->address }}" readonly />
						<input class="bg-green-700 hover:bg-green-900 text-white font-bold px-4 rounded-full" type="button" onclick="sample4_execDaumPostcode()" value="우편번호 찾기"><br>
					</div>
					
					<span id="guide" style="color:#999;display:none"></span>
					<!-- <input id="sample4_detailAddress" name="address2" class="pl-2 bg-slate-300 rounded-md" value="{{ $user->address2 }}"/> -->
				</div>
				...
				<div class="pt-12 pb-8">
					...
					<a class="bg-green-700 hover:bg-green-900 text-white font-bold py-2 px-4 rounded-full" href="{{ route('user.destroy', $user->id) }}">탈퇴</a>
					<a class="bg-green-700 hover:bg-green-900 text-white font-bold py-2 px-4 rounded-full" href="{{ route('user.index', $user->id) }}">뒤로 가기</a>
				</div>
